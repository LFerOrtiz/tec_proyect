...
VEHICLE_NAME = "agv3"                           # Name of this agv
SELF_IDS = np.array([6, 7, 8], dtype=np.int)    # ID of this agv
...
def vehicle_config_cb(msg):
    ...
    if msg.status == "coupling" and msg.vehicle_name == VEHICLE_NAME:
        all_config = True
        rospy.logerr("AGV3 full config %s", all_config)
...
if __name__ == '__main__':
    ...
    # --- Movement
    vel_pub = rospy.Publisher('agv3/cmd_vel', Twist, queue_size=1)
    move2goal = MoveToGoal(vel_pub)
    rospy.loginfo("Getting odometry...........")
    agv_odom = rospy.Subscriber('agv3/odom', Odometry, odom_cb)
    ...
    while not init_config:
        ...
        count = 0
        rospy.logdebug_once("Configuring AGV3........")

        if obtained_ids == "OBTAINED_ID":
            # --- Activate the camera and detect the ID of the front agv
            camera_sub = rospy.Subscriber('agv3/camera/image_raw', Image, tag_detection_cb)
            while not init_config:
    ...
    # --- Sensors
    camera_sub = rospy.Subscriber('agv3/camera/image_raw', Image, frame_callback)
    disconfig_sub = rospy.Subscriber('agv3/disconfig', V2VCommunication, disconfig_cb, queue_size=2)
    disconfig_pub = rospy.Publisher('agv3/disconfig', V2VCommunication, queue_size=2)
    ...
    try:
        ...
        while init_config:
            ...
            if has_id and first_call and tag_info["distance"] <= 2.5:
                ...
                angle = round(lateral_pid.compute(current_angle, 0.01, step_time, 0.05, True), 4)
