...
VEHICLE_NAME = "agv2"  # Name of this agv
SELF_IDS = np.array([3, 4, 5], dtype=np.int)  # ID of this agv
...
def frame_callback(video_frame):
    ...
    """
    global bridge, has_id, tag_info, first_call, cnt, tic
...
def odom_cb(odom_msg):
    """Odometry of the agv"""
    global x_agv_pose, y_agv_pose, agv_yaw, past_yaw, oscilated
    ...
    if math.fabs(past_yaw - agv_yaw)> 0.2:
        oscilated = True
    past_yaw = agv_yaw
...
def vehicle_config_cb(msg):
    ...
    if msg.status == "coupling" and msg.vehicle_name == VEHICLE_NAME:
        all_config = True
        rospy.logerr("AGV2 full config %s", all_config)
...
if __name__ == '__main__':
    ...
    # --- Movement
    vel_pub = rospy.Publisher('agv2/cmd_vel', Twist, queue_size=10)
    ...
    while not init_config:
        ...
        count = 0
        rospy.logdebug_once("Configuring AGV2........")
        ...
        if obtained_ids == "OBTAINED_ID":
            ...
            while not init_config:
                rospy.loginfo("ID Found: %s", FRONT_AGV_IDS)
                ...
                if ids_detect == "ID_DETECTED":
                    ...
                    rospy.loginfo("AGV2 Status: %s", config_status.status)
                    rospy.sleep(1)
                    if all_config:
    ...
    # --- Sensors
    camera_sub = rospy.Subscriber('agv2/camera/image_raw', Image, frame_callback)
    disconfig_sub = rospy.Subscriber('agv2/disconfig', V2VCommunication, disconfig_cb, queue_size=2)
    disconfig_pub = rospy.Publisher('agv2/disconfig', V2VCommunication, queue_size=2)
    ...
    try:
        ...
        while init_config:
            ...
            if has_id and first_call and tag_info["distance"] <= 2.5:
                ...
                angle = round(lateral_pid.compute(current_angle, 0.01, step_time, -0.1, False), 4)
                if math.pi + math.pi / 2 >= angle >= -(math.pi + math.pi / 2):
                    move.angular.z = ramp_vel(past_angle, angle, t_past, t_now, 2.4)
                ...
                # with open("/home/fer/Pruebas/agv2_time.csv", "a") as cvsfile:
                #     writer = csv.writer(cvsfile)
                #     writer.writerow([str(round(timeit.default_timer() - tic, 5))])
                # rospy.logwarn("Time: %s", round(timeit.default_timer() - tic, 3))
            ...
            # --- Save the angle info in a csv file with open("/home/fer/Pruebas/agv2_test.csv", "a") as cvsfile:
            # writer = csv.writer(cvsfile) writer.writerow([str(has), str(tag_info["id"]), str(round(np.degrees(
            # angle_filter[0]), 4)), str(np.degrees(front_yaw))])
